...
mod tests {
    ...
    use crate::algorithms::{
        build_bond_graph, compare_conformer_coordinates, identify_fragments, rotate_fragment,
    };
    use crate::molecule::{Atom, Molecule, OperationMode, RotationSpec};
    use crate::rotation::{is_scanning_keyword, parse_rotation_file};
    use std::fs::File;
    use std::io::Write;

    /// Create a simple butane molecule for testing
    fn create_butane_molecule() -> Molecule {
        let atoms = vec![
            Atom::new(
                "C".to_string(),
                -4.390308655000,
    ...
    fn test_fragment_identification() {
        let molecule = create_butane_molecule();
        ...
        assert!(
            frag1.contains(&0),
            "Fragment1 should contain atom 0 (first carbon)"
        );
        assert!(
            frag1.len() >= 4,
            "Fragment1 should contain at least 4 atoms (C + 3H)"
        );

        // Fragment2 should contain atom 5 (C) and its hydrogens
        assert!(
            frag2.contains(&5),
            "Fragment2 should contain atom 5 (second carbon)"
        );
        assert!(
            frag2.len() >= 3,
            "Fragment2 should contain at least 3 atoms (C + 2H)"
        );
    ...
    fn test_rotation_application() {
        ...
        assert!(
            max_diff > 0.01,
            "Rotation should change coordinates by at least 0.01 Å, got {:.6}",
            max_diff
        );
        ...
        let pivot_diff = ((coords_original[0].x - coords_rotated[0].x).powi(2)
            ...
            + (coords_original[0].z - coords_rotated[0].z).powi(2))
        .sqrt();
        assert!(
            pivot_diff < 0.001,
            "Pivot atom should not move, moved {:.6} Å",
            pivot_diff
        );
    }
    ...
    fn test_rotation_reversibility() {
        ...
        assert!(
            max_diff < 0.001,
            "Rotation should be reversible, difference: {:.6} Å",
            max_diff
        );
    }
    ...
    fn test_zero_rotation() {
        ...
        assert!(
            max_diff < 0.001,
            "Zero rotation should not change coordinates, difference: {:.6} Å",
            max_diff
        );
    }
    ...
    fn test_bond_graph_construction() {
        ...
        assert!(
            adjacency_list[0].contains(&5),
            "C-C bond should exist between atoms 0 and 5"
        );
        assert!(
            adjacency_list[5].contains(&0),
            "C-C bond should be bidirectional"
        );

        // Check that each carbon has the expected number of bonds
        assert!(
            adjacency_list[0].len() >= 4,
            "First carbon should have at least 4 bonds, has {}",
            adjacency_list[0].len()
        );
        assert!(
            adjacency_list[5].len() >= 3,
            "Second carbon should have at least 3 bonds, has {}",
            adjacency_list[5].len()
        );
    }
    ...
    fn test_scanning_keyword_recognition() {
        ...
        assert!(
            is_scanning_keyword("s"),
            "Should recognize 's' as scanning keyword"
        );
        assert!(
            is_scanning_keyword("S"),
            "Should recognize 'S' as scanning keyword"
        ...
        assert!(
            is_scanning_keyword("scan"),
            "Should recognize 'scan' as scanning keyword"
        );
        assert!(
            is_scanning_keyword("Scan"),
            "Should recognize 'Scan' as scanning keyword"
        ...
        assert!(
            !is_scanning_keyword("rotation"),
            "Should not recognize 'rotation' as scanning keyword"
        );
        assert!(
            !is_scanning_keyword("e60"),
            "Should not recognize 'e60' as scanning keyword"
        );
        assert!(
            !is_scanning_keyword("syn"),
            "Should not recognize 'syn' as scanning keyword"
        );
        assert!(
            !is_scanning_keyword("bond"),
            "Should not recognize 'bond' as scanning keyword"
        );
        assert!(
            !is_scanning_keyword(""),
            "Should not recognize empty string as scanning keyword"
        );
    }
...
"#;
        ...
        assert!(
            result.is_ok(),
            "Valid scanning syntax should parse successfully"
        );
        ...
        assert!(
            forbidden_bonds.is_empty(),
            "No forbidden bonds should be present"
        );
        assert_eq!(
            rotation_specs.len(),
            3,
            "Should have 3 scanning specifications"
        );
        assert!(
            matches!(operation_mode, OperationMode::Scanning),
            "Operation mode should be Scanning"
        );
    ...
    fn test_invalid_scanning_parameter_rejection() {
        ...
        assert!(
            result.is_err(),
            "Invalid step_size format should be rejected"
        );
        ...
        assert!(
            result.is_err(),
            "Incomplete scanning specification should be rejected"
        );
    }
...
"#;
        ...
        assert!(
            result.is_err(),
            "Mixed rotation and scanning specs should be rejected"
        );
...
"#;
        ...
        assert!(
            result.is_err(),
            "Mixed synchronous rotation and scanning specs should be rejected"
        );
...
"#;
        ...
        assert!(
            result.is_err(),
            "Mixed explicit angles and scanning specs should be rejected"
        );
    }
...
"#;
        ...
        assert!(
            result.is_ok(),
            "Configuration parameters should be inherited with scanning"
        );
        ...
        assert_eq!(
            conformer_config.max_conformers,
            Some(1000),
            "Maxgen should be inherited"
        );
        assert_eq!(
            conformer_config.auto_confirm, true,
            "Autoconfirm should be inherited"
        );
        assert!(
            matches!(operation_mode, OperationMode::Scanning),
            "Operation mode should be Scanning"
...
"#;
        ...
        assert!(
            result.is_ok(),
            "Scanning should work with forced and forbidden bonds"
        );
        ...
        assert_eq!(
            forbidden_bonds[0],
            (22, 45),
            "Forbidden bond should be (22, 45)"
        );
        assert_eq!(
            rotation_specs.len(),
            2,
            "Should have 2 scanning specifications"
        );
        assert!(
            matches!(operation_mode, OperationMode::Scanning),
            "Operation mode should be Scanning"
...
"#;
        ...
        assert!(
            result.is_ok(),
            "Pure rotation specs should parse successfully"
        );
        ...
        assert_eq!(
            rotation_specs.len(),
            3,
            "Should have 3 rotation specifications"
        );
        assert!(
            matches!(operation_mode, OperationMode::Rotation),
            "Operation mode should be Rotation"
        );
    }
...
"#;
        ...
        assert!(
            result.is_ok(),
            "Pure scanning specs should parse successfully"
        );
    ...
    fn test_calculate_distance() {
        ...
        assert!(
            ...
            "Distance should be 1.0 Å, got {:.6}",
            distance
        );
        ...
        assert!(
            (distance_3d - expected).abs() < 1e-6,
            "3D distance should be √3 ≈ {:.6}, got {:.6}",
            expected,
            distance_3d
        );
        ...
        assert!(
            zero_distance < 1e-10,
            "Distance between identical positions should be ~0, got {:.10}",
            zero_distance
        );
    }
    ...
    fn test_calculate_unit_vector() {
        ...
        assert!(
            (dx - 1.0).abs() < 1e-6,
            "Unit vector x-component should be 1.0, got {:.6}",
            dx
        );
        assert!(
            dy.abs() < 1e-6,
            "Unit vector y-component should be 0.0, got {:.6}",
            dy
        );
        assert!(
            dz.abs() < 1e-6,
            "Unit vector z-component should be 0.0, got {:.6}",
            dz
        );
        ...
        assert!(
            ...
            "Unit vector magnitude should be 1.0, got {:.6}",
            magnitude
        );
        ...
        assert!(
            (magnitude_3d - 1.0).abs() < 1e-6,
            "3D unit vector magnitude should be 1.0, got {:.6}",
            magnitude_3d
        );
        ...
        assert!(
            (dx3 - expected_component).abs() < 1e-6,
            "3D unit vector x should be 1/√3, got {:.6}",
            dx3
        );
        assert!(
            (dy3 - expected_component).abs() < 1e-6,
            "3D unit vector y should be 1/√3, got {:.6}",
            dy3
        );
        assert!(
            (dz3 - expected_component).abs() < 1e-6,
            "3D unit vector z should be 1/√3, got {:.6}",
            dz3
        );
        ...
        assert!(
            dx_zero == 0.0 && dy_zero == 0.0 && dz_zero == 0.0,
            "Unit vector for zero distance should be (0,0,0), got ({:.6}, {:.6}, {:.6})",
            dx_zero,
            dy_zero,
            dz_zero
        );
    ...
    fn test_validate_bond_length() {
        ...
        assert!(
            validate_bond_length(1.5),
            "1.5 Å should be valid (typical C-C)"
        );
        assert!(validate_bond_length(0.74), "0.74 Å should be valid (H-H)");
        assert!(validate_bond_length(2.5), "2.5 Å should be valid");
        assert!(
            validate_bond_length(0.1),
            "0.1 Å should be valid (boundary)"
        );
        assert!(validate_bond_length(10.0), "10.0 Å should be valid");
        assert!(
            validate_bond_length(10.1),
            "10.1 Å should be valid (no upper limit)"
        );
        assert!(
            validate_bond_length(100.0),
            "100.0 Å should be valid (no upper limit)"
        );

        // Test invalid bond lengths
        assert!(
            !validate_bond_length(0.09),
            "0.09 Å should be invalid (too short)"
        );
        assert!(!validate_bond_length(0.0), "0.0 Å should be invalid");
        assert!(
            !validate_bond_length(-1.0),
            "Negative bond length should be invalid"
        );
    }

    #[test]
    fn test_scan_bond_length_simple() {
        use crate::molecule::{Atom, calculate_distance, scan_bond_length};
        ...
        assert!(
            (initial_distance - 1.0).abs() < 1e-6,
            "Initial distance should be 1.0 Å"
        );
        ...
        assert!(
            (new_distance - 1.2).abs() < 1e-6,
            "New distance should be 1.2 Å, got {:.6}",
            new_distance
        );

        // Check that atom 0 (carbon) didn't move
        assert!(
            (atoms[0].x - 0.0).abs() < 1e-6,
            "Carbon x should remain 0.0"
        );
        assert!(
            (atoms[0].y - 0.0).abs() < 1e-6,
            "Carbon y should remain 0.0"
        );
        assert!(
            (atoms[0].z - 0.0).abs() < 1e-6,
            "Carbon z should remain 0.0"
        );

        // Check that hydrogen moved along x-axis
        assert!(
            (atoms[1].x - 1.2).abs() < 1e-6,
            "Hydrogen x should be 1.2, got {:.6}",
            atoms[1].x
        );
        assert!(
            atoms[1].y.abs() < 1e-6,
            "Hydrogen y should remain 0.0, got {:.6}",
            atoms[1].y
        );
        assert!(
            atoms[1].z.abs() < 1e-6,
            "Hydrogen z should remain 0.0, got {:.6}",
            atoms[1].z
        );
    }
    ...
    fn test_scan_bond_length_compression() {
        ...
        let initial_distance = calculate_distance(&atoms[0], &atoms[1]);
        assert!(
            (initial_distance - 2.0).abs() < 1e-6,
            "Initial distance should be 2.0 Å"
        );
        ...
        assert!(
            result.is_ok(),
            "Bond compression should succeed: {:?}",
            result
        );

        let new_distance = calculate_distance(&atoms[0], &atoms[1]);
        assert!(
            (new_distance - 1.5).abs() < 1e-6,
            "New distance should be 1.5 Å, got {:.6}",
            new_distance
        );

        // Check that nitrogen moved closer
        assert!(
            (atoms[1].x - 1.5).abs() < 1e-6,
            "Nitrogen x should be 1.5, got {:.6}",
            atoms[1].x
        );
    }
    ...
    fn test_scan_bond_length_3d() {
        ...
        assert!(
            (initial_distance - expected_initial).abs() < 1e-6,
            "Initial distance should be √3 ≈ {:.6}",
            expected_initial
        );
        ...
        assert!(
            result.is_ok(),
            "3D bond scanning should succeed: {:?}",
            result
        );

        let new_distance = calculate_distance(&atoms[0], &atoms[1]);
        assert!(
            (new_distance - 2.0).abs() < 1e-6,
            "New distance should be 2.0 Å, got {:.6}",
            new_distance
        );
        ...
        assert!(
            (atoms[1].x - scale_factor).abs() < 1e-6,
            "Oxygen x should be scaled correctly"
        );
        assert!(
            (atoms[1].y - scale_factor).abs() < 1e-6,
            "Oxygen y should be scaled correctly"
        );
        assert!(
            (atoms[1].z - scale_factor).abs() < 1e-6,
            "Oxygen z should be scaled correctly"
        );
    }
    ...
    fn test_scan_bond_length_multiple_fragment_atoms() {
        ...
        let initial_distance = calculate_distance(&atoms[0], &atoms[1]);
        assert!(
            (initial_distance - 1.5).abs() < 1e-6,
            "Initial C-C distance should be 1.5 Å"
        );
        ...
        assert!(
            result.is_ok(),
            "Multi-atom fragment scanning should succeed: {:?}",
            result
        );

        let new_cc_distance = calculate_distance(&atoms[0], &atoms[1]);
        assert!(
            (new_cc_distance - 2.0).abs() < 1e-6,
            "New C-C distance should be 2.0 Å, got {:.6}",
            new_cc_distance
        );
        ...
        assert!(
            (new_c_h1_dist - initial_c_h1_dist).abs() < 1e-6,
            "C-H1 distance should be preserved"
        );
        assert!(
            (new_c_h2_dist - initial_c_h2_dist).abs() < 1e-6,
            "C-H2 distance should be preserved"
        );

        // Check that all fragment atoms moved by the same displacement
        let displacement = 2.0 - 1.5; // 0.5 Å
        assert!(
            (atoms[1].x - (1.5 + displacement)).abs() < 1e-6,
            "Second carbon should move correctly"
        );
        assert!(
            (atoms[2].x - (2.5 + displacement)).abs() < 1e-6,
            "First hydrogen should move correctly"
        );
        assert!(
            (atoms[3].x - (1.5 + displacement)).abs() < 1e-6,
            "Second hydrogen should move correctly"
        );
    }
    ...
    fn test_scan_bond_length_error_conditions() {
        ...
        assert!(
            result.is_err(),
            "Should fail with invalid fragment atom index"
        );
        ...
        assert!(
            result.is_err(),
            "Should fail when atoms are at same position"
        );
    }
    ...
    fn test_end_to_end_scanning_workflow_simple() {
        ...
        use crate::molecule::{Atom, Bond, Molecule, OperationMode, RotationSpec};
        ...
        let rotation_specs = vec![RotationSpec::Scanning {
            steps: 3,
            step_size: 0.1,
        }];
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(
                    e.to_string().contains("scanning") || e.to_string().contains("Scanning"),
                    "Error should be related to scanning: {}",
                    e
                );
            }
        }
    ...
    fn test_multi_dimensional_scanning_structure() {
        ...
        // Create butane molecule for multi-dimensional scanning
        let atoms = vec![
            Atom::new(
                "C".to_string(),
                -4.390308655000,
        ...
        let result =
            generate_conformers(&mut molecule, &[], &rotation_specs, "test_multi", Some(20));
    ...
    fn test_conformer_validation_integration() {
        ...
        assert!(
            is_valid_conformer(&molecule, &atoms),
            "Valid conformer should pass validation"
        );
        assert!(
            validate_scanning_conformer(&molecule, &atoms),
            "Valid scanning conformer should pass scanning validation"
        );
        ...
        assert!(
            !is_valid_conformer(&molecule, &invalid_atoms),
            "Invalid conformer should fail validation"
        );
        ...
        assert!(
            !validate_scanning_conformer(&molecule, &long_bond_atoms),
            "Conformer with invalid bond length should fail scanning validation"
        );
    }
    ...
    fn test_scanning_mode_detection_integration() {
        ...
        let rotation_specs = vec![crate::molecule::RotationSpec::Scanning {
            steps: 2,
            step_size: 0.1,
        }];
        ...
        match result {
            ...
            Err(e) => {
                ...
                assert!(
                    error_msg.contains("scanning") || error_msg.contains("scan"),
                    "Error should be scanning-related, got: {}",
                    e
                );
            }
        }
    ...
    fn test_rotation_mode_still_works() {
        use crate::generate::generate_conformers;
        use crate::molecule::{Bond, Molecule, OperationMode, RotationSpec};

        // Ensure rotation mode still works after scanning integration
        let atoms = create_butane_molecule().atoms;
        let mut molecule = Molecule::new(atoms);
        ...
        let rotation_specs = vec![RotationSpec::Explicit(vec![0.0, 60.0, 120.0])];
        ...
        assert!(
            result.is_ok(),
            "Rotation mode should still work: {:?}",
            result
        );
    ...
    fn test_forced_forbidden_bonds_validation_errors() {
        use crate::errors::{ScanningError, validate_forced_forbidden_bonds};
        use crate::molecule::{Atom, Bond, Molecule, OperationMode};
        ...
        assert!(
            result.is_err(),
            "Forbidden bond conflict should be detected"
        );
        match result.unwrap_err() {
            ScanningError::InvalidParameters { message, .. } => {
    ...
    fn test_scanning_limits_validation_errors() {
        use crate::errors::{ScanningError, validate_scanning_limits};
        ...
        assert!(
            result.is_err(),
            "Excessive conformer count should be invalid"
        );
        match result.unwrap_err() {
            ScanningError::InvalidParameters { message, .. } => {
    ...
    fn test_skip_factor_integration_in_validation() {
        ...
        if !result_default {
            assert!(
                result_permissive || !result_permissive,
                "Permissive skip_factor test completed"
            );
        }
        ...
        if result_default {
            assert!(
                result_restrictive || !result_restrictive,
                "Restrictive skip_factor test completed"
            );
        }
...
"#;
        ...
        assert!(
            result.is_ok(),
            "Maxgen parameter should be inherited in scanning mode"
        );
        ...
        assert_eq!(
            conformer_config.max_conformers,
            Some(50),
            "Maxgen should be inherited as 50"
        );
    }
...
"#;
        ...
        assert!(
            result.is_ok(),
            "Autoconfirm parameter should be inherited in scanning mode"
        );
        ...
        assert_eq!(
            conformer_config.auto_confirm, true,
            "Autoconfirm should be inherited as true"
        );
    ...
    fn test_edge_case_error_handling() {
        ...
        assert!(
            result.is_ok(),
            "Empty fragment should be handled gracefully"
        );
        ...
        assert!(
            result.is_ok(),
            "Large molecule should be handled efficiently"
        );
    }
    ...
    #[test]
    fn test_butane_c_c_bond_scanning_end_to_end() {
        ...
        let (atoms, comment) =
            read_xyz_file("test_butane.xyz").expect("Failed to load butane test molecule");

        println!("Loaded butane: {} ({} atoms)", comment.trim(), atoms.len());
        assert_eq!(atoms.len(), 8, "butane should have 8 atoms");
        ...
        match result {
            Ok((total, valid)) => {
                println!(
                    "butane C-C scanning: {} total, {} valid conformers",
                    total, valid
                );
            ...
            Err(e) => {
                ...
                if error_msg.contains("not implemented") || error_msg.contains("placeholder") {
                    ...
                    panic!("Unexpected error in butane scanning: {}", e);
                }
            }
        }
    }
    ...
    fn test_butane_multi_dimensional_scanning_end_to_end() {
        ...
        assert!(
            bond_graph[0].contains(&1),
            "C1-C2 bond should be detected (distance: 1.54 Å)"
        );
        assert!(
            bond_graph[1].contains(&2),
            "C2-C3 bond should be detected (distance: 1.54 Å)"
        );
        assert!(
            bond_graph[2].contains(&3),
            "C3-C4 bond should be detected (distance: 1.54 Å)"
        );
        ...
        let rotation_specs = vec![
            ...
            RotationSpec::Scanning {
                steps: 4,
                step_size: -0.03,
            }, // Second bond: 4 steps (compression)
        ];
        ...
        match result {
            Ok((total, valid)) => {
                println!(
                    "Butane multi-dimensional scanning: {} total, {} valid conformers",
                    total, valid
                );
                assert!(
                    total >= 12,
                    "Should attempt at least 12 conformers (3×4 combinations)"
                );
                assert!(valid > 0, "Should generate at least some valid conformers");

                // Multi-dimensional scanning should explore more conformational space
                println!(
                    "Multi-dimensional scanning successfully generated {} combinations",
                    total
                );
            }
            Err(e) => {
                ...
                if error_msg.contains("not implemented") || error_msg.contains("placeholder") {
                    println!(
                        "Multi-dimensional scanning not fully implemented yet: {}",
                        e
                    );
                } else {
                    panic!(
                        "Unexpected error in butane multi-dimensional scanning: {}",
                        e
                    );
                }
            }
    ...
    fn test_output_file_formats_and_naming_conventions() {
        ...
        match result {
            Ok((total, valid)) => {
                println!(
                    "H2 scanning for output testing: {} total, {} valid conformers",
                    total, valid
                );
                ...
                println!(
                    "Expected files: {}_1.xyz, {}_2.xyz, ..., {}_traj.xyz",
    ...
    fn test_scanning_with_real_molecule_6cf_vc_radical() {
        ...
        if let Some((atom1, atom2)) = scanning_bond {
            println!(
                "Found C-C bond for scanning: atoms {} and {}",
                atom1 + 1,
                atom2 + 1
            );
            ...
            match result {
                Ok((total, valid)) => {
                    println!(
                        "Real molecule scanning: {} total, {} valid conformers",
                        total, valid
                    );
                    ...
                    if valid == 0 {
                        ...
                        println!(
                            "Successfully generated {} valid conformers from real molecule",
                            valid
                        );
                    }
                }
    ...
    fn test_comprehensive_validation_system_integration() {
        ...
        use crate::algorithms::{
            build_bond_graph, is_valid_conformer, validate_scanning_conformer,
        };
        use crate::io::read_xyz_file;
        use crate::molecule::{Bond, Molecule, OperationMode};
        ...
        if atoms.len() > 10 {
            molecule.add_bond(Bond {
                atom1: 1,
                atom2: 6, // C-C bond (0-based indexing)
                angles: vec![],
                is_synchronous: false,
                reference_bond: None,
                direction: 1.0,
        ...
        assert_eq!(
            bond_graph.len(),
            atoms.len(),
            "Bond graph should have entry for each atom"
        );

        // Verify that forced bond exists in bond graph if it was added
        if !molecule.forced_bonds.is_empty() && atoms.len() > 7 {
            assert!(
                bond_graph[1].contains(&6),
                "Forced bond should exist in bond graph"
            );
            assert!(
                bond_graph[6].contains(&1),
                "Forced bond should be bidirectional"
            );
        }
        ...
        println!(
            "Normal bond_factor (1.2): {} total bonds",
            total_bonds_normal / 2
        );
        println!(
            "Strict bond_factor (0.5): {} total bonds",
            total_bonds_strict / 2
        );
        ...
        assert!(
            is_valid_conformer(&molecule_permissive, &atoms),
        ...
        println!(
            "Restrictive skip_factor validation result: {}",
            restrictive_result
        );
        ...
        if !normal_result {
            assert!(
                permissive_result,
                "Permissive skip_factor should be more accepting"
            );
        }

        println!(
            "Skip_factor validation: permissive={}, normal={}, restrictive={}",
...
"#,
        ...
        assert!(
            result.is_ok(),
            "Real molecule scanning parameters should parse successfully"
        );
        ...
        assert_eq!(
            bond_factor, molecule.bond_factor,
            "Bond factor should be inherited"
        );
        assert_eq!(
            skip_factor, molecule.skip_factor,
            "Skip factor should be inherited"
        );
        assert!(
            matches!(operation_mode, OperationMode::Scanning),
            "Should be in scanning mode"
    ...
    fn test_scanning_vs_rotation_performance_comparison() {
        ...
        // Create butane molecule for performance testing
        let atoms = vec![
            Atom::new("C".to_string(), 0.0, 0.0, 0.0),
        ...
        let scanning_specs = vec![RotationSpec::Scanning {
            steps: 10,
            step_size: 0.05,
        }];

        let start_time = Instant::now();
        let scanning_result = generate_conformers(
            &mut scanning_molecule,
            &[],
            &scanning_specs,
            "perf_test_scan",
            Some(20),
        ...
        let angle_sets = vec![vec![
            0.0, 36.0, 72.0, 108.0, 144.0, 180.0, 216.0, 252.0, 288.0, 324.0,
        ]];
        let rotation_specs = vec![RotationSpec::Explicit(vec![
            0.0, 36.0, 72.0, 108.0, 144.0, 180.0, 216.0, 252.0, 288.0, 324.0,
        ])];

        let start_time = Instant::now();
        let rotation_result = generate_conformers(
            &mut rotation_molecule,
            &angle_sets,
            &rotation_specs,
            "perf_test_rot",
            Some(20),
        ...
        match (scanning_result, rotation_result) {
            (Ok((scan_total, scan_valid)), Ok((rot_total, rot_valid))) => {
                println!(
                    "Scanning: {} total, {} valid conformers",
                    scan_total, scan_valid
                );
                println!(
                    "Rotation: {} total, {} valid conformers",
                    rot_total, rot_valid
                );
                ...
                assert!(
                    scanning_duration.as_secs() < 10,
                    "Scanning should complete within 10 seconds"
                );
                assert!(
                    rotation_duration.as_secs() < 10,
                    "Rotation should complete within 10 seconds"
                );
    ...
    fn test_memory_usage_with_large_scanning_jobs() {
        ...
        match result {
            Ok((total, valid)) => {
                println!(
                    "Memory test completed: {} total, {} valid conformers",
                    total, valid
                );
                assert!(
                    total <= 60,
                    "Should not exceed theoretical maximum of 60 conformers"
                );
                ...
                println!(
                    "✓ Generated {} conformers from {} theoretical combinations",
                    total, 60
                );
            }
            Err(e) => {
                ...
                if error_msg.contains("not implemented") || error_msg.contains("placeholder") {
                    println!(
                        "Memory test skipped - scanning not fully implemented: {}",
                        e
                    );
    ...
    fn test_conformer_generation_limits_respected() {
        ...
        let result =
            generate_conformers(&mut molecule, &[], &scanning_specs, "limit_test", Some(5));

        match result {
            Ok((total, valid)) => {
                println!(
                    "Limit test: {} total, {} valid conformers (limit: 5)",
                    total, valid
                );
                ...
                if total > 5 {
                    ...
                    assert!(
                        total <= 5,
                        "Should respect conformer limit of 5, got {}",
                        total
                    );
        ...
        let result_unlimited =
            generate_conformers(&mut molecule, &[], &scanning_specs, "unlimited_test", None);

        match result_unlimited {
            Ok((total, valid)) => {
                println!(
                    "Unlimited test: {} total, {} valid conformers",
                    total, valid
                );
            ...
            Err(e) => {
                ...
                if error_msg.contains("not implemented") || error_msg.contains("placeholder") {
                    println!(
                        "Unlimited test skipped - scanning not fully implemented: {}",
                        e
                    );
                } else {
    ...
    fn test_scanning_performance_with_large_molecules() {
        ...
        println!(
            "Testing performance with large molecule ({} atoms)",
            atoms.len()
        );
        ...
        match result {
            Ok((total, valid)) => {
                println!(
                    "Large molecule test: {} total, {} valid conformers",
                    total, valid
                );
                ...
                assert!(
                    duration.as_secs() < 30,
                    "Large molecule scanning should complete within 30 seconds, took {:?}",
                    duration
                );
            ...
            Err(e) => {
                ...
                if error_msg.contains("not implemented") || error_msg.contains("placeholder") {
                    println!(
                        "Large molecule test skipped - scanning not fully implemented: {}",
                        e
                    );
                } else {
    ...
    fn test_comprehensive_unit_tests_for_scanning_functions() {
        use crate::molecule::{
            Atom, calculate_distance, calculate_unit_vector, scan_bond_length, validate_bond_length,
        };
    ...
    fn test_comprehensive_integration_tests_for_scanning_workflow() {
        ...
        match result {
            Ok((total, valid)) => {
                ...
                println!(
                    "✓ Integration test: {} total, {} valid conformers",
                    total, valid
                );
            }
            Err(e) => {
    ...
    fn test_comprehensive_edge_cases_and_error_conditions() {
        ...
        use crate::generate::generate_conformers;
        use crate::molecule::{
            Atom, Bond, Molecule, OperationMode, RotationSpec, calculate_distance, scan_bond_length,
        };
        ...
        let zero_steps_specs = vec![RotationSpec::Scanning {
            steps: 0,
            step_size: 0.1,
        }];
    ...
    fn test_comprehensive_multi_dimensional_scanning_edge_cases() {
        ...
        match result {
            Ok((total, valid)) => {
                println!(
                    "Mixed scanning: {} total, {} valid conformers",
                    total, valid
                );
        ...
        let single_step_specs = vec![RotationSpec::Scanning {
            steps: 1,
            step_size: 0.1,
        }];
        ...
        match result {
            Ok((total, valid)) => {
                println!(
                    "Single step scanning: {} total, {} valid conformers",
                    total, valid
                );
    ...
    fn test_comprehensive_validation_and_error_handling() {
        ...
        println!(
            "Permissive validation (skip_factor=0.1): {}",
            permissive_result
        );
