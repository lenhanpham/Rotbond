...
pub fn read_xyz_file(filename: &str) -> Result<(Vec<Atom>, String), Box<dyn std::error::Error>> {
    ...
    let nat_line = lines.next()
        .ok_or("Empty file")?
        .map_err(|e| format!("Failed to read atom count: {}", e))?;

    let nat = nat_line.trim().parse::<usize>()
...
/// ```
pub fn write_individual_xyz_file(filename: &str, atoms: &[Atom]) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for atom in atoms {
        writeln!(file, "{:<2} {:>15.6} {:>15.6} {:>15.6}", atom.element, atom.x, atom.y, atom.z)?;
    }
...
pub fn write_trajectory_file(
    ...
    for (idx, atoms) in conformers.iter().enumerate() {
        ...
        // Write header
        writeln!(file, "{}", atoms.len())?;
        writeln!(file, "Conformer {} of {} - generated by Rotbond", conformer_num, total_conformers)?;
...
/// # Arguments
/// 
/// * `base_name` - Base name for the file (e.g., "ethane")
...
/// 
/// let filename = format_conformer_filename("ethane", 5, 100);
/// assert_eq!(filename, "ethane_005.xyz");
/// 
/// let filename = format_conformer_filename("methane", 3, 10);
/// assert_eq!(filename, "methane_03.xyz");
/// ```
