...
use crate::molecule::{Atom, Molecule, covalent_radius, calculate_distance};
...
impl ScanningValidationContext {
    ...
    pub fn new(
        ...
        let adaptive_skip_factor =
            Self::calculate_adaptive_skip_factor(molecule, current_coordinates, scanning_bonds);
    ...
    pub fn is_scanning_bond(&self, atom1: usize, atom2: usize) -> bool {
        self.scanning_bonds
            .iter()
            .any(|&(a1, a2, _)| (a1 == atom1 && a2 == atom2) || (a1 == atom2 && a2 == atom1))
    }
...
pub fn is_valid_conformer_enhanced(
    ...
    for i in 0..n {
        for j in (i + 1)..n {
            ...
            let min_distance = (covalent_radius(&coordinates[i].element)
                + covalent_radius(&coordinates[j].element))
                * effective_skip_factor;
            ...
            if distance < min_distance {
                ...
                let diagnostic = DiagnosticMessage {
                    ...
                    message: format!(
                        ...
                        i + 1,
                        j + 1, // Convert to 1-based for user display
                    ...
                    if scanning_context.is_some() {
                        result.add_suggestion(ParameterSuggestion {
                            ...
                            suggested_value: format!(
                                "{:.2}",
                                (molecule.skip_factor - 0.1).max(0.3)
                            ),
                            reason: "Reduce steric clash sensitivity for scanning mode".to_string(),
                            expected_impact: "Should allow more conformers to pass validation"
                                .to_string(),
                        });
                    }
    ...
    if matches!(
        molecule.operation_mode,
        crate::molecule::OperationMode::Scanning
    ) {
        let scanning_result =
            validate_scanning_conformer_enhanced(molecule, coordinates, scanning_context);
...
fn validate_scanning_conformer_enhanced(
    ...
    for bond in &molecule.bonds {
        if bond.atom1 >= coordinates.len() || bond.atom2 >= coordinates.len() {
            let failure = ValidationFailure::GeometricConstraint {
                description: format!(
                    "Bond references invalid atom indices: {} or {}",
                    bond.atom1 + 1,
                    bond.atom2 + 1
                ),
        ...
        if !crate::molecule::validate_bond_length(bond_length) {
            ...
            let diagnostic = DiagnosticMessage {
                ...
                suggested_action: Some(
                    "Reduce scanning step size or check molecular structure".to_string(),
                ),
            };
            result.add_diagnostic(diagnostic);
        ...
        let min_expected_length = (covalent_radius(&atom1.element)
            + covalent_radius(&atom2.element))
            * molecule.bond_factor
            * 0.5;
        ...
        if bond_length < adjusted_min {
            ...
            let diagnostic = DiagnosticMessage {
                ...
                atom_indices: Some(vec![bond.atom1, bond.atom2]),
                suggested_action: Some(
                    "Increase scanning step size or adjust bond_factor parameter".to_string(),
                ),
            };
            result.add_diagnostic(diagnostic);
            ...
            let diagnostic = DiagnosticMessage {
                ...
                atom_indices: Some(vec![bond.atom1, bond.atom2]),
                suggested_action: Some(
                    "Reduce scanning step size or adjust bond_factor parameter".to_string(),
                ),
            };
            result.add_diagnostic(diagnostic);
        }
    ...
    for &(atom1_idx, atom2_idx) in &molecule.forced_bonds {
        if atom1_idx == 0
            || atom2_idx == 0
            || atom1_idx > coordinates.len()
            || atom2_idx > coordinates.len()
        {
        ...
        if !crate::molecule::validate_bond_length(distance) {
            ...
            let diagnostic = DiagnosticMessage {
                ...
                suggested_action: Some(
                    "Check forced bond specification or reduce scanning range".to_string(),
                ),
            };
            result.add_diagnostic(diagnostic);
        }
        ...
        let min_forced_length = (covalent_radius(&atom1.element) + covalent_radius(&atom2.element))
            * molecule.bond_factor
            * 0.3;
        let max_forced_length = (covalent_radius(&atom1.element) + covalent_radius(&atom2.element))
            * molecule.bond_factor
            * 4.0;
        ...
        if distance < min_forced_length || distance > max_forced_length {
            ...
            let diagnostic = DiagnosticMessage {
                ...
                atom_indices: Some(vec![atom1_idx - 1, atom2_idx - 1]),
                suggested_action: Some(
                    "Consider adjusting bond_factor or scanning parameters".to_string(),
                ),
            };
            result.add_diagnostic(diagnostic);
        }
    ...
    for &(atom1_idx, atom2_idx) in &molecule.forbidden_bonds {
        ...
        let bond_threshold = (covalent_radius(&atom1.element) + covalent_radius(&atom2.element))
            * molecule.bond_factor
            + 0.45;
        ...
        if distance < bond_threshold {
            ...
            let diagnostic = DiagnosticMessage {
                ...
                atom_indices: Some(vec![atom1_idx - 1, atom2_idx - 1]),
                suggested_action: Some(
                    "Reduce scanning range or adjust bond_factor to avoid forbidden bond formation"
                        .to_string(),
                ),
            };
            result.add_diagnostic(diagnostic);
...
fn generate_overall_parameter_suggestions(
    ...
    let has_bond_length_issues = result
        .diagnostic_info
        .iter()
        .any(|d| d.message.contains("bond") && d.message.contains("length"));
    ...
    if has_bond_length_issues {
        result.add_suggestion(ParameterSuggestion {
            ...
            reason:
                "Bond length validation failures suggest need for more permissive bond detection"
                    .to_string(),
    ...
    if let Some(context) = scanning_context {
        ...
        if context.adaptive_skip_factor < molecule.skip_factor * 0.8 {
            result.add_suggestion(ParameterSuggestion {
                ...
                reason: "Scanning operations may benefit from more permissive validation"
                    .to_string(),
                expected_impact: "Should improve validation success rate for scanning conformers"
                    .to_string(),
            });
        }
    }
...
fn validate_fragments(_atom1: usize, _atom2: usize, _fragment1: &[usize], _fragment2: &[usize]) {
...
pub fn rotate_fragment(
    ...
    let dihedral_info =
        get_dihedral_tracking_info(pivot_atom1, pivot_atom2, fragment, &adjacency_list, atoms);
...
fn get_dihedral_tracking_info(
    ...
    find_dihedral_reference_atoms(atom1, atom2, &fragment1, fragment, adjacency_list, atoms)
}
...
pub fn rotate_fragment_to_absolute_dihedral(
    ...
    let bond_rotation_needed = find_bond_rotation_for_dihedral(
        ...
        atom_b,
        current_dihedral,
        target_absolute_dihedral,
    );

    // Apply the calculated bond rotation
    rotate_fragment_simple(
        atoms,
        fragment,
        pivot_atom1,
        pivot_atom2,
        bond_rotation_needed,
    );
...
/// Validates conformers for scanning mode with proper exclusion of scanned bonds.
/// 
/// This function performs steric clash validation specifically designed for scanning
/// conformers. It excludes the intentionally scanned bonds from skip_factor validation
/// while still checking all other atom pairs for steric clashes.
/// 
/// # Arguments
/// 
/// * `molecule` - The molecule containing validation parameters
/// * `coordinates` - The conformer coordinates to validate
/// * `scanned_bonds` - List of (atom1, atom2) pairs that are being scanned (0-based indices)
/// 
/// # Returns
/// 
/// `true` if the conformer is valid, `false` if steric clashes are detected.
/// 
/// # Validation Logic
/// 
/// 1. **Scanned bonds**: Only check minimum length (≥ 0.1 Å), no skip_factor
/// 2. **All other atom pairs**: Apply full skip_factor validation for steric clashes
/// 3. **Result**: Accept conformer only if no non-scanned pairs have steric clashes
/// 
/// # Examples
/// 
/// ```rust
/// use rotbond::algorithms::is_valid_scanning_conformer;
/// 
/// let scanned_bonds = vec![(4, 5), (5, 9)]; // Bonds 5-6 and 6-10 (0-based)
/// if is_valid_scanning_conformer(&molecule, &coordinates, &scanned_bonds) {
///     println!("Scanning conformer is valid");
/// }
/// ```
pub fn is_valid_scanning_conformer(
    molecule: &Molecule, 
    coordinates: &[Atom],
    scanned_bonds: &[(usize, usize)]
) -> bool {
    let n = coordinates.len();
    
    // Create a set of scanned bond pairs for fast lookup (both directions)
    let mut scanned_pairs = std::collections::HashSet::new();
    for &(atom1, atom2) in scanned_bonds {
        scanned_pairs.insert((atom1, atom2));
        scanned_pairs.insert((atom2, atom1)); // Add reverse direction
    }
    
    // 1. Validate scanned bonds - only check minimum length (≥ 0.1 Å)
    for &(atom1, atom2) in scanned_bonds {
        if atom1 >= n || atom2 >= n {
            return false; // Invalid atom indices
        }
        
        let distance = calculate_distance(&coordinates[atom1], &coordinates[atom2]);
        
        // Only check minimum threshold for scanned bonds, no skip_factor
        if !crate::molecule::validate_bond_length(distance) {
            return false; // Bond too short (< 0.1 Å)
        }
    }
    
    // 2. Validate all other atom pairs with skip_factor (exclude scanned bonds)
    for i in 0..n {
        for j in (i + 1)..n {
            // Skip validation if this is a scanned bond pair
            if scanned_pairs.contains(&(i, j)) {
                continue;
            }
            ...
            // Apply skip_factor validation to non-scanned pairs
            let min_distance = (covalent_radius(&coordinates[i].element) +
                               covalent_radius(&coordinates[j].element)) * molecule.skip_factor;

            if distance < min_distance {
                return false; // Steric clash detected in non-scanned atoms
            }
        }
    }
...
/// // Currently produces no output (debug disabled)
/// ```
pub fn log_rotation_application(
    _bond_idx: usize,
    _angle: f64,
    _fragment: &[usize],
    _atoms_before: &[Atom],
...
/// ```
pub fn calculate_dihedral_angle(atom_a: &Atom, atom_b: &Atom, atom_c: &Atom, atom_d: &Atom) -> f64 {
...
fn build_bond_graph_from_atoms_with_factor(atoms: &[Atom], bond_factor: f64) -> Vec<Vec<usize>> {
    ...
    for i in 0..n {
        for j in (i + 1)..n {
            ...
            let radius_sum =
                covalent_radius(&atoms[i].element) + covalent_radius(&atoms[j].element);
...
/// ```
fn identify_fragment1(atom1: usize, atom2: usize, adjacency_list: &[Vec<usize>]) -> Vec<usize> {
    let n = adjacency_list.len();
    let mut fragment1 = Vec::new();
...
/// // For butane rotation, finds H atoms or C atoms for dihedral measurement
...
fn find_dihedral_reference_atoms(
    ...
    let atom_a = find_backbone_reference_atom(atom1, atom2, fragment1, adjacency_list, atoms)?;

    // Find atom B: bonded to atom2, in fragment2, longest backbone, not H
    let atom_b = find_backbone_reference_atom(atom2, atom1, fragment2, adjacency_list, atoms)?;
...
fn find_backbone_reference_atom(
    ...
    candidates
        .iter()
        .max_by_key(|&&atom| {
            calculate_backbone_chain_length(atom, central_atom, adjacency_list, atoms)
        })
        .copied()
}
...
fn find_bond_rotation_for_dihedral(
    ...
    for _iteration in 0..MAX_ITERATIONS {
        ...
        rotate_fragment_simple(
            &mut test_atoms_deriv,
            fragment,
            pivot_atom1,
            pivot_atom2,
            bond_rotation + DERIVATIVE_EPSILON,
        );
